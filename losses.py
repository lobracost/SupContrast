...
class SupConLossOCC(nn.Module):
    """Supervised Contrastive Loss denominator is summed over out-of-class samples only"""
    def __init__(self, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07):
        super(SupConLossOCC, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # compute log_prob
        exp_logits = torch.exp(logits) * logits_mask
        filtered_denom = exp_logits - negative_mask * exp_logits
        log_prob = logits - torch.log(filtered_denom.sum(1, keepdim=True))
...
class SupConLossDualT(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        anchor_dot_contrast_pos = torch.div(
            similarity,
            self.temperature_pos)

        # combine logits
        anchor_dot_contrast = anchor_dot_contrast_neg + mask * (anchor_dot_contrast_pos - anchor_dot_contrast_neg) 
...
class SupConLossDualTtheta(nn.Module):
    """Supervised Contrastive Loss incorporating different tau for positive and negative pairs"""
    def __init__(self, temperature_pos=0.07, contrast_mode='all',
                 pos_neg_ratio=10):
        super(SupConLossDualTtheta, self).__init__()
        self.temperature_pos = temperature_pos
        self.temperature_neg = temperature_pos / pos_neg_ratio
        self.contrast_mode = contrast_mode
...
class SupConLossOCCDualT(nn.Module):
    """Supervised Contrastive Loss denominator is summed over out-of-class samples only"""
    def __init__(self, temperature_pos=0.07, contrast_mode='all',
                 pos_neg_ratio=10):
        super(SupConLossOCCDualT, self).__init__()
        self.temperature_pos = temperature_pos
        self.temperature_neg = temperature_pos / pos_neg_ratio
        self.contrast_mode = contrast_mode
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        anchor_dot_contrast = anchor_dot_contrast + mask * anchor_dot_contrast * (
            (self.temperature_neg - self.temperature_pos) / self.temperature_pos)
